...
- Full MQTTv5 Feature Compliance, compatibility for MQTT v3.1.1 and v3.0.0:
    ...
    - Plus all the original MQTT features of MQTT v1, such as Full QoS(0,1,2), $SYS topics, retained messages, etc. 
- Developer-centric:
...
MQTT can be used as a standalone broker. Simply checkout this repository and run the [cmd/main.go](cmd/main.go) entrypoint in the [cmd](cmd) folder which will expose tcp (:1883), websocket (:1882), and dashboard (:8080) listeners.
...
Importing MQTT as a package requires just a few lines of code to get started.
``` go
import (
  "log"
...
| Type           | Import                                                                   | Info                                                                       |
|----------------|--------------------------------------------------------------------------|----------------------------------------------------------------------------|
| Access Control | [server/hooks/auth . AllowHook](hooks/auth/allow_all.go)      | Allow access to all connecting clients and read/write to  all topics.      | 
| Access Control | [server/hooks/auth . Auth](hooks/auth/auth.go)                | Rule-based access control ledger.                                          | 
| Persistence    | [server/hooks/storage/bolt](hooks/storage/bolt/bolt.go)       | Persistent storage using [BoltDB](https://dbdb.io/db/boltdb) (deprecated). | 
| Persistence    | [server/hooks/storage/badger](hooks/storage/badger/badger.go) | Persistent storage using [BadgerDB](https://github.com/dgraph-io/badger).  | 
...
### Testing
#### Unit Tests
MQTT tests over a thousand scenarios with thoughtfully hand written unit tests to ensure each function does exactly what we expect. You can run the tests using go:
```
go run --cover ./...
```
...
MQTT performance is comparable with popular brokers such as Mosquitto, EMQX, and others.
...
| --                | --             | --   | --   | --             | --   | --   |
| v2.2.10      | 124,772 | 125,456 | 124,614 | 314,461 | 313,186 | 311,910 |
...
| Broker            | publish fastest | median | slowest | receive fastest | median | slowest | 
| --                | --             | --   | --   | --             | --   | --   |
| v2.2.10      | 41,825 | 31,663| 23,008 | 144,058 | 65,903 | 37,618 |
...
| Broker            | publish fastest | median | slowest | receive fastest | median | slowest | 
| --                | --             | --   | --   | --             | --   | --   |
| v2.2.10     | 13,532 | 4,425 | 2,344 | 52,120 | 7,274 | 2,701 |
